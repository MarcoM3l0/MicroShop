...
using MicroShop.CartApi.Models;
...
public class CartRepository : ICartRepository
{
    ...
    public async Task<bool> CleanCartAsync(string userId)
    {
        ...
        {
            _context.CartItems.RemoveRange(_context.CartItems.Where(c => c.CartHeaderId == cartHeader.CartHeaderId));
        ...
        try
        {
            ...
            if (total == 1)
            {
                var cartHeader = await _context.CartHeaders.FirstOrDefaultAsync(
                                             c => c.CartHeaderId == cartItem.CartHeaderId);
    ...
    private async Task UpdateQuantityAndItems(CartDTO cartDto, Cart cart, CartHeader? cartHeader)
    {
        ...
        var cartItem = await _context.CartItems.AsNoTracking().FirstOrDefaultAsync(
                               ...
                               .ProductId && p.CartHeaderId == cartHeader.CartHeaderId);
        ...
        else
        {
            ...
            cart.CartItems.FirstOrDefault().CartItemId = cartItem.CartItemId;
    ...
    private async Task SaveProductInDataBase(CartDTO cartDto, Cart cart)
    {
        ...
        var product = await _context.Products.FirstOrDefaultAsync(p => p.ProductId ==
